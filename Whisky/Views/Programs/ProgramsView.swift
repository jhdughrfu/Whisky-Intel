...
struct ProgramsView: View {
    ...
    var body: some View {
        Form {
            DisclosureGroup("program.title", isExpanded: $areProgramsExpanded) {
                List(searchResults, id: \.self, selection: $selectedPrograms) { program in
                    ...
                    .contextMenu {
                        ...
                        if selectedPrograms.contains(program) && selectedPrograms.count > 1 {
                            ...
                            }, label: { Label("program.add.selected.blocklist", systemImage: "hand.raised") })
                            ...
                            Section {
                                Button(action: {
                                    ...
                                    blocklist = bottle.settings.blocklist
                                }, label: { Label("program.add.blocklist", systemImage: "hand.raised") })
                                .labelStyle(.titleAndIcon)
                            }
                        }
                    }
            ...
            DisclosureGroup("program.blocklist", isExpanded: $isBlocklistExpanded) {
                    ...
                    .contextMenu {
                        if selectedBlockitems.contains(blockedUrl) {
                            Button(action: {
                                ...
                                blocklist = bottle.settings.blocklist
                            }, label: { Label("program.remove.selected.blocklist", systemImage: "hand.raised") })
                            .labelStyle(.titleAndIcon)
                            .symbolVariant(.slash)
                        } else {
                            Button(action: {
                                ...
                                blocklist = bottle.settings.blocklist
                            }, label: { Label("program.remove.blocklist", systemImage: "hand.raised") })
                            .labelStyle(.titleAndIcon)
                            .symbolVariant(.slash)
                        }
                    }
        ...
        .onChange(of: bottle.settings) { _ in
            loadData()
        }
    }
    ...
    var body: some View {
        HStack {
            ...
            if showButtons {
                ...
                }, label: { Label("program.remove.blocklist", systemImage: "xmark") })
                .labelStyle(.iconOnly)
