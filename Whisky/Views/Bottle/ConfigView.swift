...
struct ConfigView: View {
    ...
    var body: some View {
        Form {
            DisclosureGroup("config.title.wine", isExpanded: $wineSectionExpanded) {
                ...
                SettingItemView(title: "config.retinaMode", loadingState: retinaModeLoadingState) {
                    Toggle("config.retinaMode", isOn: $retinaMode)
                        .onChange(of: retinaMode, perform: { newValue in
                            Task(priority: .userInitiated) {
            ...
            DisclosureGroup("config.title.dxvk", isExpanded: $dxvkSectionExpanded) {
            ...
            DisclosureGroup("config.title.metal", isExpanded: $metalSectionExpanded) {
                ...
                if let device = MTLCreateSystemDefaultDevice() {
                    if supportsApple9Family(device) {
        ...
        .onChange(of: bottle.settings.windowsVersion) { newValue in
    ...
    private func supportsApple9Family(_ device: MTLDevice) -> Bool {
        guard let apple9 = MTLGPUFamily(rawValue: 1009) else {
            return false
        }
